...
- `app/cabinet/routes/landing.py` — Python-модуль
  Классы: `LandingFeature`, `LandingTariffPeriod`, `LandingTariff`, `LandingPaymentMethodSubOption`, `LandingPaymentMethod`, `LandingDiscountInfo`, `LandingNodeInfo`, `LandingConfigResponse`, `PurchaseRequest` (1 методов), `PurchaseResponse`, `PurchaseStatusResponse`, `GiftClaimRequest` (1 методов), `GiftClaimResponse`
...
- `app/cabinet/routes/subscription_modules/purchase.py` — Python-модуль
  Классы: `TariffPurchaseContext`
  Функции: `get_purchase_options` — Get all subscription purchase options (periods, servers, traffic, devices)., `preview_purchase` — Calculate and preview the total price for selected options (classic mode only)., `submit_purchase` — Submit subscription purchase (deduct from balance, classic mode only)., `purchase_tariff` — Purchase a tariff (for tariffs mode)., `create_tariff_invoice` — Прямая оплата тарифа: инвойс на НЕДОСТАЮЩУЮ сумму без ручного пополнения., `get_trial_info` — Get trial subscription info and availability., `activate_trial` — Activate trial subscription.
...
- `app/cabinet/routes/subscription_modules/traffic.py` — Python-модуль
  Классы: нет
  Функции: `save_pending_subscription_cart`, `build_traffic_reset_status` — Build standardized status and calculations for LTE traffic reset., `get_traffic_packages` — Get available traffic packages., `purchase_traffic` — Purchase additional traffic., `save_traffic_cart` — Save cart for traffic purchase (for insufficient balance flow)., `get_traffic_reset` — Get status and calculations for LTE (White Internet) traffic reset., `perform_traffic_reset` — Reset LTE traffic usage by up to 50 GB., `save_traffic_reset_cart` — Explicitly save an LTE traffic reset cart for auto-purchase., `switch_traffic_package` — Switch to a different traffic package (change limit)., `refresh_traffic` — Refresh traffic usage from RemnaWave panel.
...
- `app/cabinet/schemas/subscription.py` — Python-модуль
  Классы: `ServerInfo`, `TrafficPurchaseInfo`, `SubscriptionData`, `SubscriptionStatusResponse`, `RenewalOptionResponse`, `RenewalRequest`, `TrafficPackageResponse`, `TrafficResetStatusResponse`, `TrafficResetResponse`, `TrafficResetSaveCartRequest`, `TrafficPurchaseRequest`, `DevicePurchaseRequest`, `AutopayUpdateRequest`, `TrialActivateRequest`, `TrialInfoResponse`, `PurchaseSelectionRequest`, `PurchasePreviewRequest`, `TariffPurchaseRequest`, `TariffInvoiceRequest`, `TariffInvoiceResponse`
  Функции: нет
...
- `app/cabinet/services/__init__.py` — Python-модуль
  Классы: нет
  Функции: нет
- `app/cabinet/services/active_invoice.py` — Python-модуль
  Классы: нет
  Функции: `is_status_cancelled` — Check if payment status denotes cancellation or failure., `is_invoice_active` — Check if payment invoice is still active (unpaid, not cancelled, within 30 min)., `get_invoice_expires_at` — Return effective expiration datetime (created_at + 30m, capped by provider expires_at if set)., `resolve_purpose` — Return (purpose_text, purpose_code: 'topup' | 'tariff')., `get_payment_url` — Extract payment URL from payment record., `build_pending_payment_response` — Convert PendingPayment to full PendingPaymentResponse with active invoice fields., `get_active_invoice_record` — Find user's current active pending payment within the 30-minute window., `same_topup_intent` — Check if existing invoice has same top-up intent., `same_tariff_intent` — Check if existing invoice has same tariff purchase intent., `send_invoice_created_telegram_message` — Send Telegram message with invoice details and Pay button., `send_invoice_cancelled_telegram_message` — Send Telegram message when invoice is cancelled., `record_cabinet_notification` — Record in-app notification in cabinet_notifications table., `cancel_pending_payment` — Cancel a user's pending payment locally.
...
- `app/handlers/subscription/traffic.py` — Python-модуль
  Классы: нет
  Функции: `handle_add_traffic`, `handle_reset_traffic`, `confirm_reset_traffic`, `handle_confirm_lte_reset` — Подтверждение и выполнение сброса расхода LTE (Белого интернета)., `refresh_traffic_config`, `get_traffic_packages_info`, `select_traffic`, `add_traffic`, `handle_no_traffic_packages`, `handle_switch_traffic`, `confirm_switch_traffic`, `execute_switch_traffic`
...
- `app/services/admin_notification_service.py` — Python-модуль
  ...
  Функции: `notify_new_client_created` — Уведомление о новом клиенте в топик NEW_USERS. Никогда не поднимает исключение.
...
- `app/services/bulk_ban_service.py` — Python-модуль
  ...
  Функции: нет
- `app/services/cabinet_purchase_notification_service.py` — Python-модуль
  Классы: нет
  Функции: `notify_telegram_user_about_cabinet_purchase` — Mirror a successful cabinet purchase to the user's Telegram bot.
...
- `app/services/monitoring_service.py` — Python-модуль
  Классы: `AutopayFailState` (2 методов), `MonitoringService` (47 методов)
...
- `app/services/recurrent_payments_service.py` — Python-модуль
  ...
  Функции: нет
- `app/services/referral_broadcast_service.py` — Python-модуль
  Классы: нет
  Функции: `get_reward_tariff_names` — Retrieve tariff names referenced by referral reward levels., `build_referral_broadcast_message` — Build dynamic referral broadcast message with DB conditions and personal links., `send_user_referral_broadcast` — Send referral broadcast with ref.png banner if Telegram, fallback to notification delivery.
...
- `app/services/wheel_service.py` — Python-модуль
  ...
  Функции: нет
- `app/services/whitelist_traffic_service.py` — Python-модуль
  Классы: `WhitelistTrafficAccountingService` (9 методов)
  Функции: нет
...
- `app/utils/photo_message.py` — Python-модуль
  Классы: нет
  Функции: `safe_edit_or_resend` — Безопасно отредактировать текст сообщения или отправить новое при ошибке., `edit_or_answer_photo` — Edit or send a photo message, falling back to text when needed.
...
- `app/utils/screen_banners.py` — Python-модуль
  Классы: нет
  Функции: `get_screen_banner` — Return cached file_id or FSInputFile for a screen banner, else None., `cache_screen_banner_file_id` — Remember Telegram file_id after a successful photo send.
...
- `app/utils/traffic_topup_limits.py` — Python-модуль
  Классы: `TrafficTopupMonthlyLimitExceeded` (1 методов)
  Функции: `is_current_calendar_month` — Return whether ``value`` is in the configured local calendar month., `get_local_month_period_key` — Return local calendar month period key, e.g. '2026-09'., `get_local_month_start_utc` — Return the UTC datetime of the start of the current local calendar month., `get_next_local_month_start_utc` — Return the UTC datetime of the start of the next local calendar month., `next_traffic_topup_at` — Return the next local calendar-month boundary when a top-up is blocked., `count_monthly_traffic_purchases` — Count regular TrafficPurchase rows created in the current local calendar month., `available_traffic_topup_gb` — Return remaining tariff top-up capacity; ``None`` means unlimited.
...
- `app/webapi/routes/public_subscription.py` — Python-модуль
  Классы: нет
  Функции: `get_public_whitelist_quota` — Return only the LTE quota belonging to the subscription URL token., `get_public_app_config` — Return the read-only app template configured in RemnaWave.
...
- `assets/banners/main.png` — файл
- `assets/banners/paid_successful.png` — файл
- `assets/banners/ref.png` — файл
- `assets/banners/sub-banner.png` — файл
...
- `migrations/alembic/versions/0110_referral_user_reward_choice.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0111_whitelist_traffic_accounting.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0112_subscription_public_host.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0113_enable_legacy_squads.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
...
- `migrations/alembic/versions/0126_tariff_is_highlighted.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0127_traffic_topup_monthly_limit.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0128_split_traffic_topup_monthly_limits.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0129_add_cabinet_notifications.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0130_traffic_reset_and_monthly_limits.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
...
- `tests/middlewares/`
- `tests/services/`
- `tests/test_cabinet_tariff_invoice.py` — Python-модуль
  Классы: нет
  Функции: `test_invoice_creates_cart_and_payment_link` — Хватает баланса на часть цены → инвойс на НЕДОСТАЮЩУЮ сумму, корзина сохранена., `test_invoice_rejects_when_balance_covers_price` — Баланс покрывает цену → 400 balance_sufficient (клиент зовёт обычную покупку)., `test_invoice_forbidden_for_restricted_user`, `test_invoice_rejects_unknown_method` — Выключенный/неизвестный метод → 400, ссылка не создаётся, корзина не пишется.
...
- `tests/test_screen_banners.py` — Python-модуль
  Классы: нет
  Функции: `banners_dir`, `test_unknown_kind_returns_none`, `test_missing_file_returns_none`, `test_known_kind_returns_fs_input_file`, `test_oversized_banner_is_resized`, `test_kind_mapping_points_at_expected_files`
...
- `tests/cabinet/__init__.py` — Python-модуль
  Классы: нет
  Функции: нет
- `tests/cabinet/test_active_invoice.py` — Python-модуль
  Классы: нет
  Функции: `test_is_invoice_active`, `test_same_intent_topup`, `test_same_intent_tariff`, `test_create_topup_first_time`, `test_create_topup_same_intent_idempotent`, `test_create_topup_different_intent_raises_409`, `test_cancel_pending_payment_success`, `test_cancel_other_user_payment_404`, `test_cancel_paid_payment_409`, `test_create_tariff_invoice_same_intent_and_409`, `test_get_notification_history`
...
- `tests/cabinet/test_admin_send_user_message.py` — Python-модуль
  Классы: нет
  Функции: `test_send_message_success`, `test_send_message_email_only_user_rejected` — Email-only юзер → 400 с кодом no_telegram_id, бот не создаётся., `test_send_message_forbidden_maps_to_400_and_closes_session` — Юзер заблокировал бота → 400 с кодом forbidden, сессия бота закрыта., `test_send_message_user_not_found`, `test_send_message_permission_registered` — users:send_message должен существовать в реестре RBAC — иначе, `test_send_email_success`, `test_send_email_no_email`, `test_send_email_empty_subject`, `test_send_email_smtp_not_configured`, `test_send_email_send_failed`
...
- `tests/cabinet/test_email_auth_gate_http.py` — Python-модуль
  Классы: нет
  Функции: `test_disabled_in_admin_refuses_register_and_login_over_http` — Админ выключил email-вход в кабинете (строка в БД), окружение говорит «включён»:, `test_enabled_in_admin_lets_request_through_the_gate` — Обратная сторона: строка 'true' в БД при выключенном окружении — запрос проходит гейт, `test_configured_test_email_login_auto_creates_verified_user_over_http` — The staging login seam creates a verified email user without SMTP.
...
- `tests/cabinet/test_traffic_packages_discount.py` — Python-модуль
  Классы: нет
  Функции: `classic_mode` — Drive ``get_traffic_packages`` down the classic (non-tariff) branch., `test_traffic_packages_expose_promo_group_discount` — A 20% traffic promo-group discount surfaces on every package., `test_traffic_packages_no_discount_when_group_has_none` — No promo discount → no discount fields, raw price unchanged., `test_traffic_packages_expose_monthly_lock`, `test_traffic_packages_respect_apply_discounts_to_addons_flag` — When the promo group opts out of addon discounts, traffic stays full price., `test_traffic_packages_apply_discount_in_tariff_mode` — Tariff-mode packages go through the same discount path as classic mode., `test_traffic_packages_hide_packages_over_tariff_topup_limit` — Packages above the tariff's remaining top-up capacity are not offered., `test_traffic_packages_floor_displayed_price_at_one_ruble` — An extreme discount never displays below 1₽ — matching POST's max(100,...) floor., `test_traffic_packages_default_group_uses_prorated_period_hint` — Default group period-based traffic discount uses the same ceil(remaining days)
- `tests/cabinet/test_traffic_reset.py` — Python-модуль
  Классы: нет
  Функции: `test_build_traffic_reset_status_calculations`, `test_reset_whitelist_subscription_traffic_logic`, `test_reset_whitelist_subscription_traffic_rejects_below_min_used`, `test_reset_whitelist_subscription_traffic_rejects_monthly_limit`, `test_add_subscription_traffic_regular_monthly_limit`, `test_cabinet_traffic_routes`, `test_auto_purchase_service_handles_traffic_reset`, `test_traffic_routes_have_no_var_keyword_params`
...
- `tests/cabinet/test_white_internet_payload.py` — Python-модуль
  Классы: нет
  Функции: `test_list_payload_exposes_independent_white_internet_usage`, `test_purchase_payload_exposes_exact_tariff_device_caps`
...
- `tests/handlers/test_web_auth_banner.py` — Python-модуль
  Классы: нет
  Функции: `test_web_auth_success_keeps_banner_and_updates_caption`
- `tests/handlers/test_whitelist_traffic_display.py` — Python-модуль
  Классы: нет
  Функции: `test_format_whitelist_traffic_uses_bytes_and_text_bar`, `test_multi_subscription_line_displays_whitelist_quota`, `test_single_subscription_detail_displays_whitelist_quota`
...
- `tests/services/test_cabinet_purchase_banner.py` — Python-модуль
  Классы: нет
  Функции: `test_purchase_mirror_prefers_banner_photo`, `test_purchase_mirror_falls_back_to_text_without_banner`, `test_purchase_mirror_falls_back_to_text_when_photo_fails`
...
- `tests/services/test_invoxy_panel_integration.py` — Python-модуль
  Классы: нет
  Функции: `test_subscription_url_uses_public_host_without_sub_path`, `test_shared_panel_filter_keeps_linked_ids_and_owned_prefix`
...
- `tests/services/test_monitoring_notification_switches.py` — Python-модуль
  Классы: нет
  Функции: `test_global_switch_stops_monitoring_notification_queries`, `test_expiration_state_updates_even_when_notifications_are_disabled`, `test_traffic_warnings_use_highest_reached_threshold_per_scope`, `test_traffic_warning_retries_same_threshold_after_delivery_failure`, `test_record_notification_is_idempotent_for_repeated_threshold`, `test_referral_broadcast_uses_seven_day_cooldown`
...
- `tests/services/test_referral_broadcast_banner.py` — Python-модуль
  Классы: нет
  Функции: `test_build_referral_broadcast_message_legacy_conditions` — Verifies that referral broadcast builds text with dynamic DB conditions and personal links., `test_build_referral_broadcast_message_cabinet_mode_button` — Verifies that in cabinet mode button opens miniapp at /referral., `test_send_user_referral_broadcast_with_banner` — Verifies that telegram user receives referral broadcast as a photo with ref.png banner.
...
- `tests/services/test_tariff_switch_policy.py` — Python-модуль
  Классы: `TestTrafficReset` (3 методов), `TestConvertedDays` (4 методов)
...
- `tests/services/test_whitelist_traffic_service.py` — Python-модуль
  Классы: нет
  Функции: `test_panel_squad_ids_accepts_uuid_and_name_forms`, `test_usage_by_user_sums_only_valid_non_negative_values`, `test_effective_whitelist_squads_removes_and_restores_access`, `test_production_seed_uses_scoped_package_prices_and_exact_device_caps`
...
- `tests/utils/test_traffic_topup_limits.py` — Python-модуль
  Классы: нет
  Функции: `test_calendar_month_limit_resets_on_the_first_day`, `test_next_topup_is_first_day_of_next_local_month`, `test_monthly_limit_is_checked_after_subscription_lock`, `test_regular_and_whitelist_monthly_limits_are_independent`
...
- `tests/webapi/test_public_subscription.py` — Python-модуль
  Классы: нет
  Функции: `test_public_quota_returns_only_scoped_quota`, `test_public_quota_rejects_invalid_token_before_database`, `test_public_quota_returns_not_found_without_lte_limit`, `test_public_quota_fails_closed_when_ip_limited`, `test_public_app_config_returns_panel_template`
